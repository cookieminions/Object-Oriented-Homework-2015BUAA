 ...
 * Objects: cityMap, reqList, startTime, taxis, RouteGuider, taxiGUI;
 * Taxi_Sys的distr方法从请求队列取出请求并实例化抢单窗口分配出租车
 * Taxi_Sys的getTaxiInfor和getTaxisAtStatus方法提供查询
 * Taxi_Sys的init_taxi方法来实例化出租车数组
 * invariant: !((cityMap==null||reqList==null||taxiGUI==null||taxis==null||RouteGuider==null)||
 * 				(cityMap.length!=80||taxis.length!=100)||(\exist int i;0<=i<80;cityMap[i].length!=80)||
 * 				(\exist int i,j;0<=i<80,0<=j<80,;cityMap[i][j]==null)||(\exist int i;0<=i<100;taxis[i]==null))
 */
public class Taxi_Sys extends Thread{
	...
	public taxi[] init_taxi() {
		/*@REQUIRES: taxis!=null; 
		  @MODIFIES: None; 
		  @EFFECTS: (\all int i; i>=0 && i<100;
		  			(taxis[i] == new taxi(cityMap,startTime,i,taxiGUI)||
		  			taxis[i] = new newtaxi(cityMap,startTime,i,taxiGUI))&&
		  			\result == taxis)
		  */
		for(int i=0;i<70;i++){
			taxis[i] = new taxi(cityMap,startTime,i,taxiGUI);
