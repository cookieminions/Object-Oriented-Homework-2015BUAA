...
public class Elevator implements SchedulableCarrier {
	...
	*/
	...
	public boolean moveUP() {
		/*
		  @MODIFIED: curStatus; 
		  @EFFECTS: (this.getCurFloor()+1>this.highLevel) ==> (\result==false);
		  			(this.getCurFloor()+1<=this.highLevel) ==> (\result==true)&&(this.curStatus==new Query(this.getCurFloor()+1, this.getCurTime()+moveTime, Query.Direction.UP));
		 */
	...
	//请根据规格补充代码
		...
		for(int i=0;i<curHandleQuery.size();i++){
			if(req.getTime()<curHandleQuery.get(i).getTime()){
	...
	public void moveForQuery() throws Exception {
		/*
		  ...
		  @EFFECTS: normal_behavior:
		  			(this.checkFinishedQuery()) ==> (this.callOpenAndClose());
		  			(\all int i;i==0||i==1||i==-1;i!=(int)Math.signum(req.getTarget()-getCurFloor())) ==> exceptional_behavior(Exception("Invalid Status."));
		  			(!Query.checkTime(getCurTime())) ==> exceptional_behavior(Exception("Time Out Of Range"));
		  */
		if(emptyQuery()) {
			return;
		}
