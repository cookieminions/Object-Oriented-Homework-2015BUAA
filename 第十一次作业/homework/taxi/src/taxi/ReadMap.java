 ...
 * Objects: cityMap, tmpMap;
 * ReadMap类通过readMap方法读取地图文件信息并初始化地图信息
 * invariant: !((cityMap==null||tmpMap==null)||(cityMap.length!=80||tmpMap.length!=80)||
 * 				(\exist int i;0<=i<80;(cityMap[i].length!=80||tmpMap[i].length!=80))||(文件("D:\\lights_map.txt")不存在))
 */
