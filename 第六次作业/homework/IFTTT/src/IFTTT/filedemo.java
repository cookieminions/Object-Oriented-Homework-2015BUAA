...
public class filedemo extends Thread{
	...
	public void run(){
		...
		//rename
		//you need [file = fileshell.rename(file,newname)] or file will not exits
		file = fileshell.rename(file, "2333.txt");
		System.out.println(file.getName());
		//move
		//you need [file = fileshell.moveTo(file,directory)] or file will not exits
		
		try {
			sleep(5000);
		} catch (InterruptedException e) {}
		
		file = fileshell.moveTo(file, "f:\\movie");
		System.out.println(file.getPath());
