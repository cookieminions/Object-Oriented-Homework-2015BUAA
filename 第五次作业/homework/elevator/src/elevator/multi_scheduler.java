...
public class multi_scheduler extends als_scheduler implements Runnable{
	...
	public void schedule() {
		...
		for(int i=0;i<3;i++){
			...
			for(int j=0;j<along_req.get(i).size();j++){
				int req_floor = along_req.elementAt(i).elementAt(j).get_des_floor();
				if(((Ele[i].get_direction().equals("UP")&&req_floor==(Ele[i].get_floor()+1))||
						(Ele[i].get_direction().equals("DOWN")&&req_floor==(Ele[i].get_floor()-1)))&&
						!Ele[i].get_status().equals("MOVE")&&!Ele[i].get_status().equals("STILL")&&Ele[i].get_flag()!=1){
					Ele[i].set_flag(1);
					//System.out.println(req_floor+" "+i+" "+Ele[i].get_floor()+" "+Ele[i].get_status());
				}
	...
	public double get_time(){
		sys_t = (System.currentTimeMillis()-start_t)/1000.0;
